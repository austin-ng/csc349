...
class TopSorter  {
    ...
    void insert_edge(int x, int y, boolean directed) 	{
        ...
        // updated my own variable in line below:
        two.indegree += 1;
        one.edges.add(two);
    ...
    public static ArrayList<Integer> topSortGenerator(String s) {
        TopSorter ts = new TopSorter(); // create new ts object
        ArrayList<Integer> result = new ArrayList<>(); // create sorted array

        try {
            ts.readfile_graph(s); // create graph using readfile function (also sets indegrees)
            Queue<Vertex> q = new LinkedList<>(); // create queue
            for (Vertex v: ts.vertices) { // add all indegree = 0 verts to queue
                if (v.indegree == 0) {
                    q.add(v);
                }
            }
            ...
            while (!q.isEmpty()) { // while queue is not empty
                temp = q.poll();
                result.add(temp.key); // add to topsort list
                for (Vertex neighbor: temp.edges) { // decrease indegree of all neighbors
                    neighbor.indegree -= 1;
                    if (neighbor.indegree == 0) { // if indegree becomes 0, add to queue
                        q.add(neighbor);
                    }
                }
                visitCount += 1; // add to visited vertices
            }
            if (visitCount != ts.nvertices) { // if there are unvisited vertices, place -1's
                for (int i = 0; i < ts.nvertices - visitCount; i++) {
                    result.add(-1);
                }
            }
